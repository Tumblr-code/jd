...
//2022-8-13 v1.2 添加支持人形傻妞与送喜官，代码统一
...
function main(){
	...
	let bind=GetBind(GetImType(),GetUserID())//获取该用户所绑定的pin
	if(bind.length==0){
		sendText("获取绑定信息失败或您未绑定本平台")
		return
	}
	for(let i=0;i<QLS.length;i++){
		...
		if(ql_token==null){
			sendText("容器"+QLS[i].name+"token获取失败,跳过\n")
			continue 
		}
		...
		for(let j=0;j<envs.length;j++){
			...
			for(let k=0;k<bind.length;k++){
				...
				if(pin==bind[k]){
					if(envs[j].status==1)
						sendText("账号"+envs[j].value.match(/(?<=pt_pin=)[^;]+/g)+"已失效")
					else{
...
function GetBind(imtype,uid){
	...
	if(imtype=="qq"){
		...
		allpins=bucketKeys("pinPGM")
		for(let i=0;i<allpins.length;i++)
			if(bucketGet("pinPGM",allpins[i])==uid)
				pin.push(allpins[i])
	}
	else if(imtype=="sxg"){
		allpins=bucketKeys("pinSXG")
		for(let i=0;i<allpins.length;i++)
			if(bucketGet("pinSXG",allpins[i])==uid)
				pin.push(allpins[i])
	}
	return pin
}
